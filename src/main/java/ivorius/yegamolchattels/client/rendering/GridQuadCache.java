...
public class GridQuadCache<T> implements Iterable<GridQuadCache.CachedQuadLevel<T>>
{
    ...
    public static <T> GridQuadCache<T> createQuadCache(int[] size, Function<Pair<BlockCoord, ForgeDirection>, T> mapper)
    {
        return createQuadCacheGreedy(size, mapper);
    }

    protected static <T> GridQuadCache<T> createQuadCacheGreedy(int[] size, Function<Pair<BlockCoord, ForgeDirection>, T> mapper)
    {
        ...
        for (Map.Entry<QuadContext<T>, CoordGrid> entry : quads)
        {
            ...
            int[] sAxes = getCacheAxes(context.direction, size[0], size[1], size[2]);
